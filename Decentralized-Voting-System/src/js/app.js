// Assuming that "../css/style.css" is important but commented out, you should include it if needed in your HTML directly.
...
window.App = {
  eventStart: function() {
    ...
    // Setup to include JWT in all jQuery AJAX requests
    $.ajaxSetup({
      beforeSend: function(xhr) {
        const token = localStorage.getItem('jwtTokenAdmin');
        console.log('Sending Token:', token); // Debugging token being sent with requests
        if (token) {
...
window.addEventListener("load", function() {
  if (typeof web3 !== 'undefined') {
    ...
    console.warn("No web3 detected. Falling back to http://localhost:9545. Consider switching to Metamask for deployment.");
    window.eth = new Web3(new Web3.providers.HttpProvider("http://127.0.0.1:9545"));
  }
  window.App.eventStart();
  console.log('Stored JWT:', localStorage.getItem('jwtTokenAdmin')); // Debugging stored token
});
